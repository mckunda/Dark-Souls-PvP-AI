...
use crate::fann_sys::fann_errno_enum::*;
use crate::fann_sys::{fann_error, fann_get_errno, fann_get_errstr};
use libc::c_int;
use std::error::Error;
use std::ffi::CStr;
use std::fmt;
...
impl FannError {
    ...
    pub unsafe fn check_no_error(errdat: *mut fann_error) -> FannResult<()> {
        ...
        Err(FannError {
            error_type,
            error_str: error_str_opt.unwrap_or_else(|_| "Invalid UTF-8 in error string".to_owned()),
        })
    }

    pub unsafe fn check_zero(
        result: c_int,
